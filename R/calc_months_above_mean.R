#' Number of months above mean annual precipitation
#'
...
#' @return Returns a graph displaying the number of months where the mean precipitation is above the mean.
...
calc_months_above_mean <- function(precip_data) {
  ...
  # Graph it!
  mean_diff_plot <- ggplot(mean_diff_calc, aes(x = water_year, y = months_above_mean)) +
    geom_bar(position="dodge", stat="identity") +
    facet_wrap(~Location) +
    theme_bw() +
    labs(x = "Water Year", y = "Number of Months Above Mean Precipitation")


  return(mean_diff_plot)
